# Build-StreamGuard.ps1
# Aya Yoki (AyaYokiVT) — Gearlight Labs
# Builds StreamGuard with obfuscation applied before packaging.
...
$SRC = "$PSScriptRoot\..\src\StreamGuard"
$OUT = "$PSScriptRoot\..\release"

Write-Host "=== StreamGuard Build Script ===" -ForegroundColor Cyan
...
dotnet publish "$SRC\StreamGuard.csproj" -c Release -r win-x64 --self-contained false -p:PublishSingleFile=false -o "$env:TEMP\sg_intermediate"
...
Copy-Item "$env:TEMP\sg_intermediate\obf_out\StreamGuard.dll" "$SRC\bin\Release\net8.0-windows\win-x64\StreamGuard.dll" -Force

# Step 4: Repack as single-file exe
Write-Host "`n[4/4] Repacking as single-file exe..." -ForegroundColor Yellow
dotnet publish "$SRC\StreamGuard.csproj" -c Release -r win-x64 --self-contained false -p:PublishSingleFile=true --no-build -o "$OUT"

Write-Host "`n=== Build complete! ===" -ForegroundColor Green
Write-Host "Output: $OUT\StreamGuard.exe"
$hash = (Get-FileHash "$OUT\StreamGuard.exe" -Algorithm SHA256).Hash
