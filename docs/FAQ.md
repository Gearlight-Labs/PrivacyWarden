...
No. The one flag is CrowdStrike's heuristic scanner at 60% confidence -- below their own threshold for a real detection. Every other vendor (Microsoft, Kaspersky, ESET, Sophos, Malwarebytes, SentinelOne, and 65+ others) says clean. Heuristic scanners flag PrivacyWarden because it does things that look suspicious in isolation: installs a Windows Service, spawns `mullvad.exe` as a subprocess, modifies DNS settings, and auto-starts a tray app on login. That's exactly what it's supposed to do. The installer also isn't signed with a commercial CA certificate (because those cost $300+/year), which makes some scanners nervous. The source code is fully public if you want to verify and build it yourself.
